		...
		if (node.connect(networkConfig)) {
			...
			if (!"auto".equalsIgnoreCase(address)) {
				try {
					...
					//TODO check ANEESH
					PeerAddress peerAddress = node.getPeer().peerBean().serverPeerAddress().withIpInternalSocket((PeerSocket4Address) PeerSocket4Address.create(inetAddress, port, port, port));
					//PeerAddress peerAddress = node.getPeer().peerBean().serverPeerAddress().withIPSocket(PeerSocketAddress.create(inetAddress, port, port, port));
...
//						.addRelayServerConfiguration(RelayType.BUFFERED_OPENTCP, tcpServer).start();
				new PeerBuilderNAT(node.getPeer().peer()).bufferTimeoutSeconds((int)bufferAge).start();
			}
			exit();
		} else {
