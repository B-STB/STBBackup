...
public class Connection implements IPeerHolder {
	...
	private boolean createPeer(INetworkConfiguration networkConfiguration) {
		try {
			...
			if (networkConfiguration.isFirewalled()) {
				PeerAddress peerAddress = peerDHT.peerAddress().withReachable4TCP(true).withReachable4UDT(true).withReachable4UDT(true).withReachable6TCP(true).withReachable6UDP(true).withReachable6UDT(true)/*.changeFirewalledTCP(true).changeFirewalledUDP(true)*/;
	...
	private PeerBuilder preparePeerBuilder(String nodeID, int port) {
		...
		serverConfig.ports(new Ports(bindPort, bindPort, bindPort));
	...
	private boolean bootstrap(INetworkConfiguration config) {
		...
		if (futureDiscover.isSuccess()) {
			logger.debug("Discovery successful. Outside ipv4 address is '{}', ipv6 address is {}, internal address is {}.", futureDiscover.peerAddress().ipv4Socket(), futureDiscover.peerAddress().ipv6Socket(), futureDiscover.peerAddress().ipInternalSocket()/*inetAddress()*/);
