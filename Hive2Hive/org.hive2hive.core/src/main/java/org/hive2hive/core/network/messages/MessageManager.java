	...
	public boolean send(BaseMessage message, PublicKey targetPublicKey) {
		...
		try {
			try {
				...
				buffer = new DataBuffer(Unpooled.wrappedBuffer(data));
			} catch (IOException e) {
			...
			FutureSend futureSend = networkManager.getConnection().getPeer()
					.send(Number160.createHash(message.getTargetKey())).dataBuffer(buffer)
	...
	public boolean sendDirect(BaseDirectMessage message, PublicKey targetPublicKey) {
		...
		try {
			...
			FutureDirect futureDirect = networkManager.getConnection().getPeer().peer()
					.sendDirect(message.getTargetAddress()).dataBuffer(buffer).start();
			// attach a future listener to log, handle and notify events
