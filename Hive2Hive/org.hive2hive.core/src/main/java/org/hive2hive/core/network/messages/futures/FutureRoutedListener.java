...
public class FutureRoutedListener extends BaseFutureAdapter<FutureSend> {
	...
	private AcceptanceReply extractAcceptanceReply(FutureSend future) {
		String errorReason = "";
		if (future.isSuccess()) {
			Collection<DataBuffer> returndedObject = future.rawDirectData1().values();
			if (returndedObject == null) {
				...
				byte[] rawData = SerializerUtil.convertToByteArray(returndedObject.iterator().next().toByteBuf());
				try {
