 ...
 * 
 */
public class DownloadChunkRunnableDHT implements Callable<Boolean> {
	...
	private static final int MAX_CHUNK_DOWNLOAD_ATTEMPTS = 25;
	...
	private Boolean doDownload() {
		boolean isSuccess = performDownload();

		if (!isSuccess) {
			if (counter++ < MAX_CHUNK_DOWNLOAD_ATTEMPTS) {
				doDownload();
			} else {
				throw new IllegalStateException("Unable to complete download as max attempts exhausted.");
			}
		} else {
			counter = 0;
		}
	...
	private Boolean performDownload() {
		...
		IParameters parameters = new Parameters().setLocationKey(metaChunk.getChunkId())
				.setContentKey(H2HConstants.FILE_CHUNK);
