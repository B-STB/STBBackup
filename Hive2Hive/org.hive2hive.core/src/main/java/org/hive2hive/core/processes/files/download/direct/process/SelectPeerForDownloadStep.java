...
//			}
//		}
		PeerSocket4Address ownInetAddress2 = task.getOwnAddress().ipInternalSocket();
		if (ownInetAddress2 != null) {
			for (PeerAddress peerAddress : addresses) {
				if (ownInetAddress2.equals(peerAddress.ipInternalSocket())) {
					// internet addresses (external IP) match, prefer this address
					// TODO: verify this assumption
					return peerAddress;
				}
		...
		PeerSocket4Address ownInetAddress = task.getOwnAddress().ipv4Socket();
		if (ownInetAddress != null) {
			for (PeerAddress peerAddress : addresses) {
				if (ownInetAddress.equals(peerAddress.ipv4Socket())) {
					// internet addresses (external IP) match, prefer this address
					// TODO: verify this assumption
					return peerAddress;
				}
		...
		PeerSocket6Address ownInetAddress3 = task.getOwnAddress().ipv6Socket();
		if (ownInetAddress3 != null) {
			for (PeerAddress peerAddress : addresses) {
				if (ownInetAddress3.equals(peerAddress.ipv6Socket())) {
					// internet addresses (external IP) match, prefer this address
					// TODO: verify this assumption
					return peerAddress;
				}
